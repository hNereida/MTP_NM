...
\section{Bit error probability $p_{eb}$}
...
It's assumed $4$ transcievers will be placed at the \SI{260}{\m} Campus Nord side, meaning the distance between transcievers is $d = \SI{86.67}{\m}$. The central frequency of operation must be between \SI{2.4}{\GHz} and \SI{2.525}{\GHz}; the middle value \SI{2.4625}{\GHz} is taken.
...
Now, from \Cref{fig:Luque-GFSK-png} and testbed measurements it's derived that to get $p_{eb} = \SI{1e-3}{}$, $\frac{E_b}{N_0} = \SI{13}{\dB}$. If there's \SI{17}{\dB} difference between the received power and the sensitivity for which $p_{eb} = \SI{1e-3}{}$, then $p_{eb}$ is obtained by looking at $\frac{E_b}{N_0} = \SI{30}{\dB}$. 
...
\begin{equation}
	...
	\end{split} \ .
\end{equation}
In this case, the difference of the received power with respect to the senstivity level is $\SI{-98.564}{\dB}\text{m} - \left( - \SI{104}{\dB}\text{m} \right) = \SI{5.44}{\dB}$. Adding up the \SI{13}{\dB}, the \Cref{fig:Luque-GFSK-png} must be analyzed at \SI{18.55}{\dB}. In this case, by extrapolating the testbed measurements plot a safe value to take is
\begin{equation}
	p_{eb} = \SI{1e-5}{} \ .
\end{equation}
...
\begin{equation}
	...
	\end{split} \ .
\end{equation}
This means that the number of retries must be
\begin{equation}
	R = 4 \ .
\end{equation}
...
\subsection{TOKEN packet, 3 nodes distance}
...
In conclusion, it's been justified that for both the DATA and TOKEN packets a total of $T=5$ tries, i.e., $R=4$ retries are required, to meet the set thresholds for the \SI{260}{\m} distance.
